import numpy as np
from As.src.utils.helpers import preds_to_integer
import torch
import json
import cv2
import collections
...
def Recognition(imgs_word_imgs):
    ...
    """
    model = torch.jit.load("As/checkpoints/model.pth")
    decoder = json.load(open("As/checkpoints/model_decoder.json"))
    model.eval()
